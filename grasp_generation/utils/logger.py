...
class Logger:
    ...
    def log(self, energy, E_fc, E_dis, E_pen, E_spen, E_joints, E_oopen, step, show=False):
        ...
        # self.writer.add_scalar('Energy/normal', E_normal.mean(), step)
        self.writer.add_scalar('Energy/E_oopen', E_oopen.mean(), step)
        ...
        if show:
            print(f'Step %d energy: %f  fc: %f  dis: %f  pen: %f  spen: %f  joints: %f oopen: %f' % (step, energy.mean(), E_fc.mean(), E_dis.mean(), E_pen.mean(), E_spen.mean(), E_joints.mean(), E_oopen.mean()))
