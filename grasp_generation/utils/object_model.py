...
class ObjectModel:
    ...
    def __init__(self, data_root_path, batch_size_each, num_samples=2000, device="cuda", scale=[0.05]):
    ...
    def initialize(self, object_code_list):
        ...
        assert len(object_code_list) == len(self.scale_choice)
        for i, object_code in enumerate(object_code_list):
            ...
            self.object_scale_tensor.append(self.scale_choice[i] * torch.ones(self.batch_size_each, device=self.device))
            if object_code in graspem_object_list:
                self.object_mesh_list.append(tm.load(os.path.join(self.data_root_path, object_code, f'{object_code}.stl')))
            else:
            ...
            # self.object_mesh_list.append(tm.load(os.path.join('../data/object_mesh', f'{object_code}.obj'), force="mesh", process=False))
    ...
    def get_object_diameter(self):
        ...
        for i in range(len(self.object_mesh_list)):
            ...
            # diameter.append(torch.norm(torch.from_numpy(mesh.bounds[1]) - torch.from_numpy(mesh.bounds[0])))
            diameter.append(torch.min(torch.from_numpy(mesh.bounds[1]) - torch.from_numpy(mesh.bounds[0])))
        diameter = torch.stack(diameter, dim=0).to(self.device).repeat_interleave(self.batch_size_each, dim=0) * self.object_scale_tensor.reshape(-1) # (n_objects * batch_size_each)
        return diameter
