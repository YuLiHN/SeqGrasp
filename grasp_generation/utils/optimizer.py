...
class Annealing:
    def __init__(self, hand_model, switch_possibility=0.1, starting_temperature=3, temperature_decay=0.95, annealing_period=10,
        ...
        if self.hand_model.n_dofs != 45:
            self.ema_grad_hand_pose = torch.zeros(self.hand_model.n_dofs + 9, dtype=torch.float, device=device)
        else:
            self.ema_grad_hand_pose = torch.zeros(self.hand_model.n_dofs + 6, dtype=torch.float, device=device)

    def try_step(self, joint_mask=None):
        ...
        hand_pose = self.hand_model.hand_pose - \
            step_size * self.hand_model.hand_pose.grad / (torch.sqrt(self.ema_grad_hand_pose) + 1e-6) * joint_mask
        ...
        contact_pairs = contact_pairs = [[*np.random.choice(sublist[0], n_contact//2).tolist(), *np.random.choice(sublist[1], n_contact//2).tolist()] for sublist in self.hand_model.contact_pool_list]
        new_contact_point_indices = torch.tensor(contact_pairs).to(self.device)
        contact_point_indices[switch_mask] = new_contact_point_indices[switch_mask]
